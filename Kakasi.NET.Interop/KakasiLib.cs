...
namespace KakasiNET
{
	...
	public class KakasiLib : MarshalByRefObject
	{
		...
		private const string DllName = @"libkakasi.dll";
		...
		/// </summary>
		private IntPtr _kakasiLibPtr = IntPtr.Zero;
		...
		public void Init(string executionPath)
		{
			...
			var path = Path.Combine(kakasiLibPath, DllName);
			// Load library
			_kakasiLibPtr = LoadLibrary(path);

			// Check for errors
			if (_kakasiLibPtr != IntPtr.Zero)
			{

				// Loaded correctly
				_kakasiGetoptArgv =
						(KakasiGetoptArgv)
						Marshal.GetDelegateForFunctionPointer(GetProcAddress(_kakasiLibPtr, "kakasi_getopt_argv"),
								typeof(KakasiGetoptArgv));

				_kakasiDo =
						(KakasiDo)
						Marshal.GetDelegateForFunctionPointer(GetProcAddress(_kakasiLibPtr, "kakasi_do"), typeof(KakasiDo));
		...
		public void SetParams(string[] @params)
		{
			...
			// Init, if required
			if (_kakasiLibPtr == IntPtr.Zero) Init();
