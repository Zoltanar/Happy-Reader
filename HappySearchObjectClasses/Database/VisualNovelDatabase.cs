...
namespace Happy_Apps_Core.Database
{
    public class VisualNovelDatabase : IDisposable
    {
        ...
        /// <summary>
        /// If DB file does not already exist, first looks into file with same name (optionally zipped) in executable folder, 
        /// and copies it if it exists, else, creates new database file.
        /// </summary>
        private void ProcessFirstTime(string dbFile)
        {
            if (File.Exists(dbFile)) return;
            var databaseFileName = Path.GetFileName(dbFile);
            var executableFolder = Directory.GetParent(Assembly.GetExecutingAssembly().Location).FullName;
            var localDbFile = Path.Combine(executableFolder, databaseFileName);
            var localZipDbFile = Path.Combine(executableFolder, Path.ChangeExtension(databaseFileName, ".zip"));
            if (!File.Exists(localDbFile) && File.Exists(localZipDbFile))
            {
                try
                {
                    StaticHelpers.UnzipSingleFile(localZipDbFile, localDbFile, true);
                }
                catch(Exception ex)
                {
                    StaticHelpers.Logger.ToFile($"Failed to unzip local DB file: {localZipDbFile} to {localDbFile}");
            ...
            {
                StaticHelpers.Logger.ToFile($"DB file did not exist in target path but found local file, copying...");
                try
                {
                    File.Copy(localDbFile, dbFile);
                }
                catch ( IOException ex)
                {
                    StaticHelpers.Logger.ToFile($"Failed to copy local DB file: {localDbFile} to {dbFile}");
                    StaticHelpers.Logger.ToFile(ex);
                    throw;
                }
            }
