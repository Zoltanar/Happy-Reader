...
namespace DatabaseDumpReader
{
    public static class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            var result = Execute(out var errorMessage);
            Console.WriteLine($"Update: {result}");
            if(!string.IsNullOrWhiteSpace(errorMessage)) Console.WriteLine($"Error: {errorMessage}");
            return (int)result;
        }

        public static ExitCode Execute(out string errorMessage)
        {
            ExitCode result;
            ...
            try
            {
                ...
                result = Run(dumpFolder, StaticHelpers.CSettings.UserID, out downloadWatch, out runWatch, out errorMessage);
            ...
            finally
            {
                ...
                if (downloadWatch != null) PrintLogLine([$"Time for Dump Download: {downloadWatch.Elapsed.TotalMinutes:00}:{downloadWatch.Elapsed.Seconds:00}"]);
                if (runWatch != null) PrintLogLine([$"Time for DB Update: {runWatch.Elapsed.TotalMinutes:00}:{runWatch.Elapsed.Seconds:00}"]);
                if (syncWatch != null) PrintLogLine([$"Time for Image Sync: {syncWatch.Elapsed.TotalMinutes:00}:{syncWatch.Elapsed.Seconds:00}"]);
            }
            return result;
        ...
        public static ExitCode Run(string dumpFolder, int userId, out Stopwatch downloadWatch, out Stopwatch runWatch, out string resultMessage)
        {
            runWatch = null;
            ...
            if (result is ExitCode.ReloadLatest or ExitCode.Update)
            {
                ...
                {
                    resultMessage = "VNDB update was rejected.";
                    return ExitCode.NoUpdate;
                }
                else
                {
        ...
        {
            anyDownload = false;
            var dbDownloaded = StaticHelpers.DownloadFile(LatestDbDumpUrl, DumpFolder, null, out var dbDumpPath, ref anyDownload, PrintLogLine);
            ...
            while (true)
            {
                ...
                if (size != 0 && !name.Equals("./", StringComparison.InvariantCulture))
                {
                    ...
                    do
                    {
                        ...
                        readSoFar += stream.Read(buf, 0, buf.Length);
