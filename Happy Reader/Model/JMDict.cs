...
namespace Happy_Reader
{
    public class JMDict
    {
        ...
        private readonly Deinflection _deinflectionEngine = new();
        private static readonly bool UseDeinflections = false;

        public void ReadFiles(string folder)
        {
            _dictionaryTerms = Array.Empty<Term>();
            _kanjiTerms = new Dictionary<string, Kanji>();
            Dictionary<string, DeinflectionReason[]> deinflections = null;
            ...
            StaticHelpers.Logger.ToFile("Reading files for JMDict, KanjiDic, Deinflections...");
            ...
            StaticHelpers.Logger.ToFile($"JMDict: Loaded {_dictionaryTerms.Length:N0} terms, {_kanjiTerms.Count:N0} kanji, {_deinflectionEngine.ReasonsCount} deinflection reasons.");
        }
        ...
        public readonly struct Term : ITerm
        {
            ...
            public override string ToString() => $"{Expression} - {Reading} - {string.Join(", ", Glossary)}";
        }

        internal string GetTags(string tags, bool kanjiTags)
        {
