...
namespace Happy_Apps_Core_Tests
{
	...
	{
		private readonly VndbConnection _conn = new VndbConnection((message, severity) => Debug.WriteLine($"Severity: {severity}\t {message}"), null, null, null);
		private readonly MethodInfo _tryQueryNoReplyMethod;

		public VndbApiTests()
		{
			_conn.Login(new VndbConnection.LoginCredentials(ClientName, ClientVersion), printCertificates: false);
		...
		public async Task GetVN()
		{
			...
			//details
			Assert.AreEqual("https://s.vndb.org/cv/39/20339.jpg", vn.ImageId, "Failed at details, Payload: " + _conn.LastResponse.JsonPayload);
			//tags
	...
	public class VndbApiLoginTests
	{
		...
		{
			_conn.Login(new VndbConnection.LoginCredentials(ClientName, ClientVersion, "hacTest", "hacTest".ToCharArray()), printCertificates: false);
